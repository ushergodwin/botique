   ...
   <nav class="navbar navbar-expand-md navbar-dark bg-dark sticky-top p-3">
    ...
    <div class="collapse navbar-collapse" id="navbarCollapse">
        <div class="navbar-nav">
            ...
            <a href="settings" class="nav-item nav-link"><i class="fas fa-shopping-basket"></i> Canceled Orders <small><span class="badge badge-light current" id="top-badge"><?php echo $cancelled->get_cancelled(); ?></span></small></a>
        </div>
...
font-weight-bold">Loggedin as: <?php echo $user->get_firstName(); ?></span>&nbsp;<span id="time" class="text-light"></span></h5> &nbsp; &nbsp; <a href="#" class="text-light">Home /<small class="text-warning">Admin</small> / <small class="text-secondary">Clients</small> &nbsp; &nbsp;  <small id="loading"><span class="spinner-border spinner-border-sm"></span> loadingData</small> </a> </div><a href="#hide-bottom-bar.asp" class="nav-item nav-link d-flex flex-column
...
<div class="w3-container">  
    ...
    <h5 class="card-header py-1"> <i class="fas fa-shopping-basket"></i> CANCELLED ORDERS <small><span class="badge badge-light current"><?php echo $cancelled->get_cancelled(); ?></span></small></h5>
    <div class="card-body">
      <div class="table-responsive">
  <?php 
    $sql = "SELECT image, name, quantity, price, it_total_price, ordered.order_ID, shipping, payment, total_cost, status, ordered_at, firstname, lastname, contact, location from all_cart inner join ordered on all_cart.order_ID = ordered.order_ID inner join users on ordered.email = users.email INNER JOIN address ON users.email = address.email where ordered.status='Order Cancelled' order by ordered_at desc";
...
echo "</table>";
        ...
        echo "No Orders that are cancelled Yet!";
    }
echo"</div>";
    mysqli_close($con);
    ?>
