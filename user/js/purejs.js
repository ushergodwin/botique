 ...
 let numbers = /^[0-9-+ ]*$/;
   if(!cont.value.match(numbers)){ 
	...
	if(!uname.value.match(mailformat)){
     ...
     let alphernumeric = /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9])(?=.*?[^\w\s]).{8,}$/;
if (!pass.value.match(alphernumeric)){
    var f = document.getElementById("pass").innerHTML ="Password should contain at least 1 upercase letter, 1 lowercase letter, 1 number  and specail charcter like !$£%*&@";
pass.focus();
return false;
 }
